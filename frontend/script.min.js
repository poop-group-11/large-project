var connection,sessionId,hooks=[],hookList=new Object;function startGame(){document.getElementById("start").style.display="none",document.getElementById("back").style.display="none",document.getElementById("settings-menu").style.display="none",document.getElementById("settings-button").style.display="none",document.getElementById("session-code").style.display="none",connection.emit("sessionStart",sessionId),initGame(),draw()}function createUser(e){hooks[userCount]=new Hook(e,userCount),hookList[e]=userCount++,ctx.clearRect(0,0,ctx.canvas.width,ctx.canvas.height);for(var n=0;n<userCount;n++)hooks[n].origin={x:ctx.canvas.width/(userCount+1)*(hooks[n].id+1)-hooks[n].img.width/2,y:0},hooks[n].x=hooks[n].origin.x-hooks[n].img.width/2,hooks[n].y=hooks[n].origin.y+hooks[n].length,hooks[n].draw()}function newSession(){document.getElementById("home-screen").style.display="none",document.getElementById("game-screen").style.display="grid",document.getElementById("settings-menu").style.display="none",document.getElementById("game-music").play(),connection=io("https://poopgroup11.xyz"),fetch("https://poopgroup11.xyz/api/openSession",{method:"POST"}).then(e=>e.json()).then(function(e){1==e.success?(sessionId=e.message,connection.emit("joinSession",sessionId),connection.on("browserJoined",consoleCheck),document.getElementById("session-code").innerHTML=sessionId,connection.on("userJoined",createUser)):backToMenu()}).catch(e=>console.log(e)),loadAssets()}function backToMenu(){document.getElementById("home-screen").style.display="flex",document.getElementById("game-screen").style.display="none",document.getElementById("settings-menu").style.display="none",document.getElementById("game-music").pause()}function displayPopUp(e,n){document.getElementById(e).style.display=n}function consoleCheck(){console.log("Successful Connection!")}
